...
const statusColors: Record<string, string> = {
  ACTIVE: 'bg-[#9ED36A]/10 text-[#9ED36A] border-green-200',
  ...
  CONFIRMED: 'bg-[#9ED36A]/10 text-[#9ED36A] border-green-200',
...
function PaymentBanner() {
  ...
  if (payment === 'success') {
    return (
      <div className="mb-6 px-4 py-3 rounded-xl bg-[#9ED36A]/10 border border-green-200">
          ...
          <button
            ...
            className={`px-5 py-2.5 rounded-xl text-sm font-semibold transition-all whitespace-nowrap ${
              tab === t
                ...
                : 'text-[#6B7C86] hover:text-[#1a1a1a] hover:bg-slate-100'
            }`}
          >
          ...
          <>
                        ...
                        <div key={b.id} className="px-5 sm:px-6 py-4 flex flex-col sm:flex-row sm:items-center justify-between hover:bg-slate-50 transition-colors gap-2 border-b border-slate-50 last:border-0">
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2.5 flex-wrap">
                              <span className={`badge border ${statusColors[b.status] || 'bg-slate-100 text-[#9AADB8]'}`}>
                                {b.status.replace('_', ' ')}
                              </span>
            ...
            {tab === 'listings' && user.canCarry && (
              <div className="space-y-4">
                {listings.length === 0 ? (
                  <div className="bg-[#162E3D] rounded-2xl border border-slate-200 p-10 sm:p-12 text-center">
                  ...
                  listings.map(l => (
                    <div key={l.id} className="bg-[#162E3D] rounded-2xl border border-slate-200 p-5 sm:p-6 card-hover">
            ...
            {tab === 'bookings' && (
              <div className="space-y-4">
                {bookings.length === 0 ? (
                  ...
                  bookings.map(b => (
                    <div key={b.id} className="bg-[#162E3D] rounded-2xl border border-slate-200 p-5 sm:p-6 card-hover">
                      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
                        <div>
                          <div className="flex items-center gap-2.5 mb-2 flex-wrap">
                        ...
                        <div className="sm:text-right">
                          ...
                          {b.paymentStatus && (
                            <span className={`inline-block text-[10px] font-bold uppercase tracking-wider mt-1 px-2 py-0.5 rounded ${
                              ...
                              b.paymentStatus === 'FAILED' ? 'bg-red-500/10 text-red-700' :
                          ...
                          <div className="mt-2 flex gap-2 justify-end flex-wrap">
                            <a
                              ...
                              className="text-[11px] font-medium text-[#FF6A2A] hover:text-[#a87a3d] transition-colors"
                            >
                              Download Invoice
                            </a>
                            {(b.status === 'DELIVERED' || b.status === 'COMPLETED') && (
                              <Link
                                ...
                                className="text-[11px] font-medium text-[#6B7C86] hover:text-[#1a1a1a] transition-colors"
                              >
                                Book Again
                              </Link>
                            )}
...
function StatCard({ label, value, accent }: { label: string; value: string; accent?: boolean }) {
  return (
    <div className={`rounded-2xl border p-5 sm:p-6 transition-shadow hover:shadow-md ${accent ? 'bg-[#FF6A2A] border-[#FF6A2A]' : 'bg-[#162E3D] border-slate-200'}`}>
