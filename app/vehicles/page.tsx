...
export default function VehiclesPage() {
  ...
  if (authLoading) return <div className="min-h-screen bg-[#FAF8F4]" />
  if (!user) return (
    <div className="min-h-screen bg-[#FAF8F4] flex items-center justify-center">
      <p className="text-stone-500">Please <Link href="/login" className="text-[#C6904D]">sign in</Link> to manage vehicles</p>
    </div>
  )

  const inputClass = 'w-full px-3 py-2 rounded-lg border border-[#DED7CC] text-sm focus:border-[#C6904D] focus:ring-2 focus:ring-[#C6904D]/10 outline-none'

  return (
    <div id="main-content" className="min-h-screen bg-[#FAF8F4] py-8 px-4">
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-2xl font-bold text-[#1d1916]" style={{ fontFamily: 'var(--font-display)' }}>My Vehicles</h1>
        ...
        {showAdd && (
          <div className="bg-white rounded-lg border border-[#DED7CC] p-5 mb-6">
            <h2 className="font-semibold text-[#1d1916] mb-4">Add Vehicle</h2>
        ...
        {loading ? (
          <div className="space-y-4">{[1,2].map(i => <div key={i} className="h-28 bg-white rounded-lg border border-[#DED7CC] animate-pulse" />)}</div>
        ) : vehicles.length === 0 ? (
          <div className="text-center py-16 bg-white rounded-lg border border-[#DED7CC]">
            <p className="text-stone-500 mb-4">No vehicles added yet</p>
            <button onClick={() => setShowAdd(true)} className="text-[#C6904D] font-medium hover:underline">Add your first vehicle</button>
          </div>
        ) : (
          <div className="space-y-4">
            {vehicles.map(v => (
              <div key={v.id} className="bg-white rounded-lg border border-[#DED7CC] p-5">
                {editingId === v.id ? (
                  <div>
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-4">
                    ...
                    <div className="flex gap-2">
                      ...
                      <button onClick={() => setEditingId(null)} className="px-3 py-1.5 border border-[#DED7CC] rounded text-xs font-medium">Cancel</button>
                    </div>
                  </div>
                ) : (
                  <div className="flex items-start justify-between">
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="font-semibold text-[#1d1916]">{v.make} {v.model}</span>
                        {v.year && <span className="text-xs text-stone-400">({v.year})</span>}
                        <span className="px-2 py-0.5 bg-stone-100 rounded text-xs text-stone-500">{v.vehicleType}</span>
                    ...
                    <div className="flex gap-2">
                      <button onClick={() => { setEditingId(v.id); setEditForm(v) }} className="px-3 py-1.5 border border-[#DED7CC] rounded text-xs font-medium text-[#1d1916] hover:bg-[#FAF8F4]">Edit</button>
