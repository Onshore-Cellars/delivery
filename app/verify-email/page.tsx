    ...
    return (
      <div className="page-container flex items-center justify-center py-20">
        <div className="text-center">
          ...
          <p className="text-[var(--c-text-3)] mt-4">Verifying your email...</p>
        </div>
      </div>
    )
  }
      ...
      <div className="text-center">
        <div className={`w-16 h-16 rounded-full ${success ? 'bg-[var(--c-success)]/10' : 'bg-[#B23A2E]/10'} flex items-center justify-center mx-auto mb-4`}>
          {success ? (
            ...
            <svg className="w-8 h-8 text-[var(--c-error)]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
          )}
        </div>
        <h2 className="text-xl font-bold text-[var(--c-ink)] mb-2">{success ? 'Email Verified!' : 'Verification Failed'}</h2>
        <p className="text-[var(--c-text-3)] mb-6">{success ? 'Your email has been verified. You can now use all features.' : error}</p>
