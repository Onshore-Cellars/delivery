      ...
      <div className="max-w-5xl mx-auto">
        ...
        {loading ? (
          ...
          <>
            {/* Carrier Stats */}
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
              <StatCard label="Total Revenue" value={fmt(data.carrier.totalRevenue)} color="text-green-600" />
              <StatCard label="Last 30 Days" value={fmt(data.carrier.revenue30d)} color="text-green-600" />
            ...
            <div className="grid md:grid-cols-2 gap-6 mb-8">
              ...
              <div className="bg-white rounded-lg border border-[#e8e4de] p-5">
                  ...
                  {data.carrier.monthlyRevenue.map(m => (
                    <div key={m.month} className="flex items-center gap-3">
                      ...
                      <div className="flex-1 bg-slate-100 rounded-full h-6 relative overflow-hidden">
                        ...
                        />
                        <span className="absolute inset-0 flex items-center justify-end pr-2 text-[10px] font-medium text-slate-600">
                    ...
                    {data.carrier.topRoutes.map((r, i) => (
                      <div key={r.route} className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <span className="w-5 h-5 rounded-full bg-[#C6904D]/10 text-[#C6904D] text-[10px] font-bold flex items-center justify-center">{i + 1}</span>
            ...
            {/* Listing Performance */}
            <div className="bg-white rounded-lg border border-[#e8e4de] p-5 mb-8">
          ...
          <>
            {/* Shipper Stats */}
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
              <StatCard label="Total Spent" value={fmt(data.shipper.totalSpent)} color="text-blue-600" />
              <StatCard label="Last 30 Days" value={fmt(data.shipper.spent30d)} color="text-blue-600" />
              <StatCard label="Completed" value={String(data.shipper.totalShipments)} />
              <StatCard label="Active" value={String(data.shipper.activeShipments)} color="text-amber-600" />
            </div>
            ...
            <div className="flex items-center gap-8">
              <div>
                ...
                <div className="flex items-center gap-1 mt-1">
                  {[1,2,3,4,5].map(s => (
                    <span key={s} className={`text-lg ${s <= Math.round(data.rating.average) ? 'text-[#C6904D]' : 'text-slate-200'}`}>★</span>
                  ))}
                </div>
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  <span className="text-sm text-slate-500">30-day avg:</span>
