...
export default function ListingDetailPage() {
  ...
  const [bookingForm, setBookingForm] = useState({
    ...
    specialHandling: '', deliveryAddress: '', yachtName: '', marinaName: '', deliveryNotes: '',
  })
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mt-6 grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {/* Route */}
            <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-6">
              <h2 className="font-bold text-navy-900 mb-4">Route</h2>
              <div className="flex items-center gap-6">
                <div className="flex-1">
                  <div className="text-xs text-slate-400 uppercase tracking-wider">Origin</div>
                  <div className="text-lg font-bold text-navy-900">{listing.originPort}</div>
                ...
                <div className="flex flex-col items-center px-4">
                  ...
                  <svg className="w-5 h-5 text-gold-500 mt-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 8l4 4m0 0l-4 4m4-4H3" /></svg>
                </div>
                <div className="flex-1">
                  <div className="text-xs text-slate-400 uppercase tracking-wider">Destination</div>
                  <div className="text-lg font-bold text-navy-900">{listing.destinationPort}</div>
              ...
              <div className="mt-4 grid grid-cols-2 gap-4 pt-4 border-t border-slate-100">
                ...
                {listing.estimatedArrival && (
                  <div>
                    ...
                    <div className="font-semibold text-navy-900">{formatDate(listing.estimatedArrival)}</div>
                  </div>
                )}
              </div>
            </div>
            ...
            {/* Capacity */}
            <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-6">
              <h2 className="font-bold text-navy-900 mb-4">Capacity</h2>
              <div className="mb-4">
                <div className="flex justify-between text-sm mb-1">
                  <span className="text-slate-500">Fill rate</span>
                  <span className="font-semibold text-navy-900">{fillPercent}%</span>
                </div>
                <div className="w-full h-3 bg-slate-100 rounded-full overflow-hidden">
                  <div className="h-full bg-gradient-to-r from-navy-600 to-navy-400 rounded-full transition-all" style={{ width: `${fillPercent}%` }} />
                </div>
              </div>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                <div><div className="text-xs text-slate-400">Available Weight</div><div className="text-lg font-bold text-navy-900">{listing.availableKg.toFixed(0)}kg</div></div>
                <div><div className="text-xs text-slate-400">Available Volume</div><div className="text-lg font-bold text-navy-900">{listing.availableM3.toFixed(1)}m&sup3;</div></div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-6 sticky top-24">
              ...
              {listing.flatRate ? (
                <div className="text-3xl font-extrabold text-navy-900">{formatCurrency(listing.flatRate, listing.currency)}<span className="text-sm font-normal text-slate-400 ml-1">flat</span></div>
              ) : (
                <div className="space-y-1">
                  {listing.pricePerKg && <div className="text-xl font-bold text-navy-900">{formatCurrency(listing.pricePerKg, listing.currency)}<span className="text-sm font-normal text-slate-400">/kg</span></div>}
                  {listing.pricePerM3 && <div className="text-xl font-bold text-navy-900">{formatCurrency(listing.pricePerM3, listing.currency)}<span className="text-sm font-normal text-slate-400">/m&sup3;</span></div>}
                </div>
              )}
              ...
              <div className="mt-6 space-y-3">
                {user && user.id !== listing.carrier.id && (
                  <>
                    <button onClick={() => { setShowBooking(!showBooking); setShowBid(false); setShowContact(false) }} className="w-full btn-gold text-sm !py-3">
                      Book Space
                    </button>
              ...
              <div className="mt-6 pt-6 border-t border-slate-100">
                <div className="flex items-center gap-3 mb-3">
                  <div className="w-10 h-10 rounded-full bg-navy-100 flex items-center justify-center">
                    <span className="text-sm font-semibold text-navy-700">{listing.carrier.name.split(' ').map(n => n[0]).join('')}</span>
                  </div>
                  <div>
                    <div className="font-semibold text-navy-900 text-sm">{listing.carrier.name}</div>
                ...
                {listing.carrier.avgRating > 0 && (
                  <div className="flex items-center gap-1 mb-2">
                    {[1, 2, 3, 4, 5].map(star => (
                      <svg key={star} className={`w-4 h-4 ${star <= Math.round(listing.carrier.avgRating) ? 'text-gold-500' : 'text-slate-200'}`} fill="currentColor" viewBox="0 0 20 20">
                ...
                {listing.carrier.phone && user && (
                  <a href={`tel:${listing.carrier.phone}`} className="mt-3 flex items-center gap-2 text-sm text-navy-600 hover:text-navy-800 transition-colors">
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-navy-950/60 backdrop-blur-sm">
            <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto">
              ...
              <form onSubmit={submitBooking} className="p-6 space-y-4">
                <div><label className="block text-sm font-medium text-navy-900 mb-1">Cargo *</label><input required className="w-full px-4 py-2.5 rounded-lg border border-slate-200 text-sm outline-none focus:border-navy-400" placeholder="e.g. Provisions for MY Serenity" value={bookingForm.cargoDescription} onChange={e => setBookingForm({...bookingForm, cargoDescription: e.target.value})} /></div>
                <div className="grid grid-cols-2 gap-3">
                  <div><label className="block text-sm font-medium text-navy-900 mb-1">Weight (kg) *</label><input type="number" required step="0.1" max={listing.availableKg} className="w-full px-4 py-2.5 rounded-lg border border-slate-200 text-sm outline-none focus:border-navy-400" value={bookingForm.weightKg} onChange={e => setBookingForm({...bookingForm, weightKg: e.target.value})} /></div>
                  <div><label className="block text-sm font-medium text-navy-900 mb-1">Volume (m&sup3;) *</label><input type="number" required step="0.1" max={listing.availableM3} className="w-full px-4 py-2.5 rounded-lg border border-slate-200 text-sm outline-none focus:border-navy-400" value={bookingForm.volumeM3} onChange={e => setBookingForm({...bookingForm, volumeM3: e.target.value})} /></div>
                </div>
                <div><label className="block text-sm font-medium text-navy-900 mb-1">Yacht Name</label><input className="w-full px-4 py-2.5 rounded-lg border border-slate-200 text-sm outline-none focus:border-navy-400" placeholder="e.g. MY Serenity" value={bookingForm.yachtName} onChange={e => setBookingForm({...bookingForm, yachtName: e.target.value})} /></div>
                <div><label className="block text-sm font-medium text-navy-900 mb-1">Marina / Berth</label><input className="w-full px-4 py-2.5 rounded-lg border border-slate-200 text-sm outline-none focus:border-navy-400" placeholder="e.g. Port Hercules, Berth 42" value={bookingForm.marinaName} onChange={e => setBookingForm({...bookingForm, marinaName: e.target.value})} /></div>
                <div><label className="block text-sm font-medium text-navy-900 mb-1">Special Handling</label><input className="w-full px-4 py-2.5 rounded-lg border border-slate-200 text-sm outline-none focus:border-navy-400" placeholder="e.g. Refrigerated, fragile, temperature-controlled" value={bookingForm.specialHandling} onChange={e => setBookingForm({...bookingForm, specialHandling: e.target.value})} /></div>
                <button type="submit" disabled={formLoading} className="w-full btn-gold !py-3 text-sm disabled:opacity-50">{formLoading ? 'Confirming...' : 'Confirm Booking'}</button>
              </form>
            </div>
          </div>
        )}
        ...
        {showBid && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-navy-950/60 backdrop-blur-sm">
            <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full">
              <div className="p-6 border-b border-slate-100 flex justify-between items-center">
                <h2 className="text-xl font-bold text-navy-900">Place a Bid</h2>
              ...
              <form onSubmit={submitBid} className="p-6 space-y-4">
                ...
                <div><label className="block text-sm font-medium text-navy-900 mb-1">Your Bid ({listing.currency}) *</label><input type="number" required step="0.01" min={listing.minBidPrice || 0} className="w-full px-4 py-2.5 rounded-lg border border-slate-200 text-sm outline-none focus:border-navy-400" value={bidForm.amount} onChange={e => setBidForm({...bidForm, amount: e.target.value})} /></div>
                <div className="grid grid-cols-2 gap-3">
                  <div><label className="block text-sm font-medium text-navy-900 mb-1">Weight (kg) *</label><input type="number" required step="0.1" className="w-full px-4 py-2.5 rounded-lg border border-slate-200 text-sm outline-none focus:border-navy-400" value={bidForm.weightKg} onChange={e => setBidForm({...bidForm, weightKg: e.target.value})} /></div>
                  <div><label className="block text-sm font-medium text-navy-900 mb-1">Volume (m&sup3;) *</label><input type="number" required step="0.1" className="w-full px-4 py-2.5 rounded-lg border border-slate-200 text-sm outline-none focus:border-navy-400" value={bidForm.volumeM3} onChange={e => setBidForm({...bidForm, volumeM3: e.target.value})} /></div>
                </div>
                <div><label className="block text-sm font-medium text-navy-900 mb-1">Message</label><textarea rows={3} className="w-full px-4 py-2.5 rounded-lg border border-slate-200 text-sm outline-none focus:border-navy-400 resize-none" placeholder="Tell the carrier about your cargo..." value={bidForm.message} onChange={e => setBidForm({...bidForm, message: e.target.value})} /></div>
        ...
        {showContact && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-navy-950/60 backdrop-blur-sm">
            <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full">
              <div className="p-6 border-b border-slate-100 flex justify-between items-center">
                <h2 className="text-xl font-bold text-navy-900">Contact {listing.carrier.name}</h2>
              ...
              <form onSubmit={sendMessage} className="p-6 space-y-4">
                {listing.carrier.phone && (
                  <a href={`tel:${listing.carrier.phone}`} className="flex items-center gap-3 p-3 rounded-lg border border-slate-200 hover:bg-slate-50 transition-colors">
                    <svg className="w-5 h-5 text-navy-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" /></svg>
                    <div><div className="font-medium text-navy-900 text-sm">Call {listing.carrier.name.split(' ')[0]}</div><div className="text-xs text-slate-400">{listing.carrier.phone}</div></div>
                  </a>
                )}
                {listing.carrier.email && (
                  <a href={`mailto:${listing.carrier.email}?subject=RE: ${listing.title}`} className="flex items-center gap-3 p-3 rounded-lg border border-slate-200 hover:bg-slate-50 transition-colors">
                    <svg className="w-5 h-5 text-navy-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>
                    <div><div className="font-medium text-navy-900 text-sm">Email</div><div className="text-xs text-slate-400">{listing.carrier.email}</div></div>
                  </a>
                )}
                <div className="pt-2 border-t border-slate-100">
                  <label className="block text-sm font-medium text-navy-900 mb-1">Send In-App Message</label>
                  <textarea rows={4} required className="w-full px-4 py-2.5 rounded-lg border border-slate-200 text-sm outline-none focus:border-navy-400 resize-none" placeholder="Hi, I'm interested in your listing..." value={messageContent} onChange={e => setMessageContent(e.target.value)} />
                </div>
