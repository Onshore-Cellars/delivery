...
const statusConfig: Record<string, { label: string; classes: string }> = {
  ...
  EXPIRED: { label: 'Expired', classes: 'bg-slate-50 text-[#6B7C86] border border-slate-200' },
  CANCELLED: { label: 'Cancelled', classes: 'bg-red-500/10 text-red-400 border border-red-200' },
}
    ...
    <div className="page-container">
        ...
        {showNewForm && (
          <div className="bg-[#162E3D] rounded-xl shadow-sm border border-slate-100 p-6 mb-6">
            ...
            {createError && (
              <div className="mb-4 px-4 py-3 rounded-lg bg-red-500/10 border border-red-100">
            ...
            <form onSubmit={handleCreateQuote} className="space-y-4">
              ...
              <div>
                ...
                <input
                  type="text"
                  required
                  className="w-full px-4 py-2.5 rounded-lg border border-slate-200 text-sm text-[#1a1a1a] focus:border-[#FF6A2A] focus:ring-2 focus:ring-[#FF6A2A]/10 outline-none"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    className="w-full px-4 py-2.5 rounded-lg border border-slate-200 text-sm text-[#1a1a1a] focus:border-[#FF6A2A] focus:ring-2 focus:ring-[#FF6A2A]/10 outline-none bg-[#162E3D]"
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <button type="button" onClick={addPackage} className="text-xs font-semibold text-[#FF6A2A] hover:text-[#b07d3f]">+ Add Package</button>
                </div>
                {packages.length === 0 && (
                  ...
                  return (
                    <div key={pkg.id} className="flex flex-wrap items-end gap-2 mb-2 p-3 bg-slate-50 rounded-lg border border-slate-100">
                      <div className="w-36">
                        ...
                        <select
                          className="w-full px-2 py-1.5 rounded border border-slate-200 text-xs bg-[#162E3D]"
                          value={pkg.type}
                      ...
                      <div className="w-16">
                        ...
                        <input type="number" min="1" max="2400" disabled={typeInfo?.isPallet} className={"w-full px-2 py-1.5 rounded border text-xs" + (typeInfo?.isPallet ? ' bg-slate-100 text-[#6B7C86] border-slate-200 cursor-not-allowed' : ' border-slate-200')} value={pkg.lengthCm} onChange={e => updatePackage(pkg.id, 'lengthCm', parseInt(e.target.value) || 0)} />
                      </div>
                      <div className="w-16">
                        <label className="block text-[10px] text-[#6B7C86] mb-0.5">W cm</label>
                        <input type="number" min="1" max="2400" disabled={typeInfo?.isPallet} className={"w-full px-2 py-1.5 rounded border text-xs" + (typeInfo?.isPallet ? ' bg-slate-100 text-[#6B7C86] border-slate-200 cursor-not-allowed' : ' border-slate-200')} value={pkg.widthCm} onChange={e => updatePackage(pkg.id, 'widthCm', parseInt(e.target.value) || 0)} />
                      </div>
                      <div className="w-16">
              ...
              <div className="flex justify-end gap-3 pt-2">
                <button
                  ...
                  className="px-5 py-2.5 text-sm font-medium text-[#9AADB8] hover:bg-slate-100 rounded-lg transition-colors"
                >
                  Cancel
                </button>
                <button
        ...
        <div className="flex gap-1 mb-6 bg-[#162E3D] rounded-xl shadow-sm border border-slate-100 p-1">
          <button
            ...
            className={`flex-1 py-2.5 px-4 rounded-lg text-sm font-semibold transition-colors ${
              activeTab === 'received'
                ...
                : 'text-[#6B7C86] hover:text-[#1a1a1a] hover:bg-slate-50'
            }`}
          >
            Received
            {receivedQuotes.length > 0 && (
              <span className={`ml-2 inline-flex items-center justify-center w-5 h-5 rounded-full text-[10px] font-bold ${
                activeTab === 'received' ? 'bg-[#162E3D]/20 text-white' : 'bg-slate-100 text-[#9AADB8]'
              }`}>
          ...
          >
            ...
            {sentQuotes.length > 0 && (
              <span className={`ml-2 inline-flex items-center justify-center w-5 h-5 rounded-full text-[10px] font-bold ${
                activeTab === 'sent' ? 'bg-[#162E3D]/20 text-white' : 'bg-slate-100 text-[#9AADB8]'
              }`}>
                {sentQuotes.length}
              </span>
            )}
        ...
        {/* Error */}
        {error && (
          <div className="mb-6 px-4 py-3 rounded-lg bg-red-500/10 border border-red-100">
                        ...
                        return (
                          <div className="flex flex-wrap gap-1.5 mb-3">
                            {pkgs.map((p, i) => {
                              ...
                              return (
                                <span key={i} className="inline-flex items-center gap-1 px-2 py-0.5 rounded bg-slate-100 text-[11px] font-medium text-[#9AADB8]">
                        ...
                        {(isRequester || isProvider) &&
                          (quote.status === 'PENDING' || quote.status === 'QUOTED') && (
                            <button
                              ...
                              disabled={actionLoading === quote.id}
                              className="px-3 py-1.5 text-xs font-medium text-red-400 hover:bg-red-500/100/10 rounded-lg transition-colors disabled:opacity-50"
                            >
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
                        <div>
                          ...
                          <input
                            ...
                            min="0"
                            required
                            className="w-full px-3 py-2 rounded-lg border border-slate-200 text-sm text-[#1a1a1a] focus:border-[#FF6A2A] focus:ring-2 focus:ring-[#FF6A2A]/10 outline-none bg-[#162E3D]"
