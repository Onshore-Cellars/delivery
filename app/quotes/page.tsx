  ...
  return (
    <div className="page-container">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            <p className="text-[11px] font-semibold text-[var(--c-accent)] uppercase tracking-[0.15em] mb-1">Pricing</p>
            <h1 className="text-xl sm:text-2xl font-semibold text-[var(--c-ink)] tracking-[-0.02em]">Quotes</h1>
            <p className="text-sm text-[var(--c-text-3)] mt-1">Request, manage, and respond to shipping quotes</p>
          </div>
          <button
          ...
          <div className="bg-[var(--c-surface)] rounded-xl shadow-sm border border-[var(--c-border)] p-6 mb-6">
            ...
            <form onSubmit={handleCreateQuote} className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-[var(--c-ink)] mb-1.5">Origin (Port, Address or Company) *</label>
                  <PortAutocomplete
                ...
                <div>
                  <label className="block text-sm font-medium text-[var(--c-ink)] mb-1.5">Destination (Port, Address or Company) *</label>
                  <PortAutocomplete
              ...
              <div>
                <label className="block text-sm font-medium text-[var(--c-ink)] mb-1.5">Cargo Description *</label>
                <input
                  type="text"
                  required
                  className="w-full px-4 py-2.5 rounded-lg border border-black/10 text-sm text-[var(--c-ink)] focus:border-[var(--c-accent)] focus:ring-2 focus:ring-[var(--c-accent)]/10 outline-none"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-[var(--c-ink)] mb-1.5">Cargo Type</label>
                  <select
                    className="w-full px-4 py-2.5 rounded-lg border border-black/10 text-sm text-[var(--c-ink)] focus:border-[var(--c-accent)] focus:ring-2 focus:ring-[var(--c-accent)]/10 outline-none bg-[var(--c-surface)]"
                ...
                <div>
                  <label className="block text-sm font-medium text-[var(--c-ink)] mb-1.5">Preferred Date</label>
                  <input
                    type="date"
                    className="w-full px-4 py-2.5 rounded-lg border border-black/10 text-sm text-[var(--c-ink)] focus:border-[var(--c-accent)] focus:ring-2 focus:ring-[var(--c-accent)]/10 outline-none"
              ...
              <div>
                  ...
                  return (
                    <div key={pkg.id} className="flex flex-wrap items-end gap-2 mb-2 p-3 bg-[var(--c-surface)] rounded-lg border border-[var(--c-border)]">
                      <div className="w-36">
                        ...
                        <select
                          className="w-full px-2 py-1.5 rounded border border-black/10 text-xs bg-[var(--c-surface)]"
                          value={pkg.type}
                      ...
                      <div className="w-16">
                        <label className="block text-[10px] text-[var(--c-text-3)] mb-0.5">Qty</label>
                        <input type="number" min="1" className="w-full px-2 py-1.5 rounded border border-black/10 text-xs" value={pkg.quantity} onChange={e => updatePackage(pkg.id, 'quantity', parseInt(e.target.value) || 1)} />
                      </div>
                      <div className="w-20">
                        <label className="block text-[10px] text-[var(--c-text-3)] mb-0.5">Wt (kg)</label>
                        <input type="number" min="0" step="0.1" className="w-full px-2 py-1.5 rounded border border-black/10 text-xs" value={pkg.weightKg} onChange={e => updatePackage(pkg.id, 'weightKg', parseFloat(e.target.value) || 0)} />
                      </div>
                      {/* Dimensions — always shown; L/W locked for pallets */}
                      <div className="w-16">
                        <label className="block text-[10px] text-[var(--c-text-3)] mb-0.5">L cm{typeInfo?.isPallet ? '' : ''}</label>
                        <input type="number" min="1" max="2400" disabled={typeInfo?.isPallet} className={"w-full px-2 py-1.5 rounded border text-xs" + (typeInfo?.isPallet ? ' bg-[var(--c-canvas-2)] text-[var(--c-text-3)] border-black/10 cursor-not-allowed' : ' border-black/10')} value={pkg.lengthCm} onChange={e => updatePackage(pkg.id, 'lengthCm', parseInt(e.target.value) || 0)} />
                      </div>
                      <div className="w-16">
                        <label className="block text-[10px] text-[var(--c-text-3)] mb-0.5">W cm</label>
                        <input type="number" min="1" max="2400" disabled={typeInfo?.isPallet} className={"w-full px-2 py-1.5 rounded border text-xs" + (typeInfo?.isPallet ? ' bg-[var(--c-canvas-2)] text-[var(--c-text-3)] border-black/10 cursor-not-allowed' : ' border-black/10')} value={pkg.widthCm} onChange={e => updatePackage(pkg.id, 'widthCm', parseInt(e.target.value) || 0)} />
                      </div>
                      <div className="w-16">
                        <label className="block text-[10px] text-[var(--c-text-3)] mb-0.5">H cm{typeInfo?.isPallet ? ' ✎' : ''}</label>
                        <input type="number" min="1" max="2400" className="w-full px-2 py-1.5 rounded border border-black/10 text-xs" value={pkg.heightCm} onChange={e => updatePackage(pkg.id, 'heightCm', parseInt(e.target.value) || 0)} />
                      </div>
                      <div className="flex-1 min-w-[100px]">
                        <label className="block text-[10px] text-[var(--c-text-3)] mb-0.5">Note</label>
                        <input type="text" className="w-full px-2 py-1.5 rounded border border-black/10 text-xs" placeholder="Optional" value={pkg.description} onChange={e => updatePackage(pkg.id, 'description', e.target.value)} />
                      </div>
                      <button type="button" onClick={() => removePackage(pkg.id)} className="p-1.5 text-[var(--c-text-2)] hover:text-[var(--c-error)] transition-colors">
                      ...
                      {pkg.lengthCm > 0 && pkg.widthCm > 0 && pkg.heightCm > 0 && (
                        <div className="w-full">
                          <p className="text-[10px] text-[var(--c-text-2)] mt-0.5">
                            ...
                            {pkg.quantity > 1 && (
                              <> · <strong className="text-[var(--c-accent)]">{(calcCubicMetres(pkg.lengthCm, pkg.widthCm, pkg.heightCm) * pkg.quantity).toFixed(4)} m³ total</strong></>
                            )}
                          </p>
                        </div>
                      )}
                ...
                {packages.length > 0 && (
                  <div className="flex gap-4 text-xs text-[var(--c-text-3)] mt-1">
                    <span>Total weight: <strong className="text-[var(--c-ink)]">{totalWeight.toFixed(1)} kg</strong></span>
                    <span>Total volume: <strong className="text-[var(--c-ink)]">{totalVolume.toFixed(2)} m&sup3;</strong></span>
                  </div>
                )}
              </div>

              <div>
                <label className="block text-sm font-medium text-[var(--c-ink)] mb-1.5">Special Requirements</label>
                <input
                  type="text"
                  className="w-full px-4 py-2.5 rounded-lg border border-black/10 text-sm text-[var(--c-ink)] focus:border-[var(--c-accent)] focus:ring-2 focus:ring-[var(--c-accent)]/10 outline-none"
              ...
              <div className="flex justify-end gap-3 pt-2">
                <button
                  ...
                  className="px-5 py-2.5 text-sm font-medium text-[var(--c-text-2)] hover:bg-[var(--c-canvas-2)] rounded-lg transition-colors"
                >
                  Cancel
                </button>
                <button
        ...
        <div className="flex gap-1 mb-6 bg-[var(--c-surface)] rounded-xl shadow-sm border border-[var(--c-border)] p-1">
          ...
          >
            ...
            {receivedQuotes.length > 0 && (
              <span className={`ml-2 inline-flex items-center justify-center w-5 h-5 rounded-full text-[10px] font-bold ${
                activeTab === 'received' ? 'bg-[var(--c-surface)]/20 text-white' : 'bg-[var(--c-canvas-2)] text-[var(--c-text-2)]'
              }`}>
          ...
          >
            ...
            {sentQuotes.length > 0 && (
              <span className={`ml-2 inline-flex items-center justify-center w-5 h-5 rounded-full text-[10px] font-bold ${
                activeTab === 'sent' ? 'bg-[var(--c-surface)]/20 text-white' : 'bg-[var(--c-canvas-2)] text-[var(--c-text-2)]'
              }`}>
                {sentQuotes.length}
              </span>
            )}
        ...
        {loading ? (
          ...
          <div className="bg-[var(--c-surface)] rounded-xl shadow-sm border border-[var(--c-border)] p-16 text-center">
            ...
            <p className="text-[var(--c-text-3)] mb-2">
              ...
              return (
                <div key={quote.id} className="bg-[var(--c-surface)] rounded-xl shadow-sm border border-[var(--c-border)] overflow-hidden">
                  <div className="p-6">
                    ...
                    <div className="flex items-start justify-between mb-4">
                      <div className="flex items-center gap-3 min-w-0">
                        <div className="flex items-center gap-2 min-w-0">
                          <span className="font-semibold text-[var(--c-ink)] truncate">{quote.originPort}</span>
                          <svg className="w-4 h-4 text-[var(--c-text-2)] flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 8l4 4m0 0l-4 4m4-4H3" />
                          </svg>
                          <span className="font-semibold text-[var(--c-ink)] truncate">{quote.destinationPort}</span>
                        </div>
                      </div>
                    ...
                    {/* Cargo details */}
                    <p className="text-sm text-[var(--c-text-2)] mb-3">{quote.cargoDescription}</p>
                        ...
                        return (
                          <div className="flex flex-wrap gap-1.5 mb-3">
                            {pkgs.map((p, i) => {
                              ...
                              return (
                                <span key={i} className="inline-flex items-center gap-1 px-2 py-0.5 rounded bg-[var(--c-canvas-2)] text-[11px] font-medium text-[var(--c-text-2)]">
                                  {p.quantity > 1 ? `${p.quantity}x ` : ''}{typeLabel}
                                  {p.weightKg > 0 && <span className="text-[var(--c-text-2)]">&middot; {(p.weightKg * p.quantity).toFixed(0)}kg</span>}
                                </span>
                              )
                            })}
                          </div>
                    ...
                    <div className="flex flex-wrap gap-x-6 gap-y-2 text-sm mb-4">
                      <div>
                        <span className="text-[var(--c-text-2)]">Weight:</span>{' '}
                        <span className="font-medium text-[var(--c-ink)]">{quote.weightKg.toFixed(1)} kg</span>
                      </div>
                      <div>
                        <span className="text-[var(--c-text-2)]">Volume:</span>{' '}
                        <span className="font-medium text-[var(--c-ink)]">{quote.volumeM3.toFixed(1)} m&sup3;</span>
                      </div>
                      {quote.cargoType && (
                        <div>
                          <span className="text-[var(--c-text-2)]">Type:</span>{' '}
                          <span className="font-medium text-[var(--c-ink)]">{quote.cargoType}</span>
                        </div>
                      )}
                      {quote.preferredDate && (
                        <div>
                          <span className="text-[var(--c-text-2)]">Preferred:</span>{' '}
                          <span className="font-medium text-[var(--c-ink)]">{formatDate(quote.preferredDate)}</span>
                        </div>
                      )}
                    </div>

                    {quote.specialRequirements && (
                      <p className="text-xs text-[var(--c-text-2)] mb-4 italic">
                    ...
                    {quote.quotedPrice != null && (
                      <div className="bg-[var(--c-surface)] rounded-lg p-4 border border-black/10 mb-4">
                        <div className="flex items-center justify-between">
                          <div>
                            <div className="text-xs text-[var(--c-accent)] uppercase tracking-wider mb-0.5">Quoted Price</div>
                          ...
                          {quote.validUntil && (
                            <div className="text-right">
                              <div className="text-xs text-[var(--c-accent)] uppercase tracking-wider mb-0.5">Valid Until</div>
                              <div className="text-sm font-semibold text-[var(--c-ink)]">{formatDate(quote.validUntil)}</div>
                            </div>
                          )}
                        </div>
                        {quote.responseMessage && (
                          <p className="text-sm text-[var(--c-ink)] mt-3 pt-3 border-t border-black/10">
                            {quote.responseMessage}
                          </p>
                        )}
                      </div>
                    ...
                    <div className="flex items-center justify-between pt-4 border-t border-[var(--c-border)]">
                      <div className="flex items-center gap-2 text-sm text-[var(--c-text-3)]">
                        {otherParty && (
                          <>
                            <div className="w-6 h-6 rounded-full bg-[var(--c-surface)] flex items-center justify-center flex-shrink-0">
                              <span className="text-[10px] font-semibold text-[var(--c-ink)]">
                                {otherParty.name.split(' ').map(n => n[0]).join('')}
                              </span>
                            </div>
                            <span>{isRequester ? 'Provider' : 'Requester'}: <span className="font-medium text-[var(--c-ink)]">{otherParty.name}</span></span>
                            {otherParty.company && <span className="text-[var(--c-text-2)]">&middot; {otherParty.company}</span>}
                          </>
                        )}
                        {!otherParty && (
                          <span className="text-[var(--c-text-2)]">Open request &middot; no provider assigned</span>
                        )}
                        <span className="text-[var(--c-text-3)] ml-2">&middot;</span>
                        <span className="text-[var(--c-text-2)]">{formatDate(quote.createdAt)}</span>
                      </div>
                  ...
                  {isRespondingToThis && (
                    <div className="bg-[var(--c-surface)] border-t border-[var(--c-border)] p-6">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
                        <div>
                          <label className="block text-xs font-medium text-[var(--c-ink)] mb-1">Price (EUR) *</label>
                          <input
                            type="number"
                            step="0.01"
                            min="0"
                            required
                            className="w-full px-3 py-2 rounded-lg border border-black/10 text-sm text-[var(--c-ink)] focus:border-[var(--c-accent)] focus:ring-2 focus:ring-[var(--c-accent)]/10 outline-none bg-[var(--c-surface)]"
                        ...
                        <div>
                          <label className="block text-xs font-medium text-[var(--c-ink)] mb-1">Valid Until</label>
                          <input
                            type="date"
                            className="w-full px-3 py-2 rounded-lg border border-black/10 text-sm text-[var(--c-ink)] focus:border-[var(--c-accent)] focus:ring-2 focus:ring-[var(--c-accent)]/10 outline-none bg-[var(--c-surface)]"
                        ...
                        <div>
                          <label className="block text-xs font-medium text-[var(--c-ink)] mb-1">Message</label>
                          <input
                            type="text"
                            className="w-full px-3 py-2 rounded-lg border border-black/10 text-sm text-[var(--c-ink)] focus:border-[var(--c-accent)] focus:ring-2 focus:ring-[var(--c-accent)]/10 outline-none bg-[var(--c-surface)]"
                      ...
                      <div className="flex justify-end gap-2">
                        <button
                          ...
                          className="px-4 py-2 text-xs font-medium text-[var(--c-text-2)] hover:bg-[var(--c-surface)] rounded-lg transition-colors"
                        >
                          Cancel
                        </button>
                        <button
