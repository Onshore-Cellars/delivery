...
const statusConfig: Record<string, { label: string; classes: string }> = {
  ...
  EXPIRED: { label: 'Expired', classes: 'bg-[#162E3D] text-[#6B7C86] border border-white/10' },
  CANCELLED: { label: 'Cancelled', classes: 'bg-red-900/20 text-red-400 border border-red-500/30' },
}
...
export default function QuotesPage() {
  ...
  return (
    <div className="page-container">
        ...
        {/* New Quote Form */}
        {showNewForm && (
          <div className="bg-[#162E3D] rounded-xl shadow-sm border border-slate-100 p-6 mb-6">
            ...
            <form onSubmit={handleCreateQuote} className="space-y-4">
              ...
              <div>
                ...
                <input
                  type="text"
                  required
                  className="w-full px-4 py-2.5 rounded-lg border border-white/10 text-sm text-[#F7F9FB] focus:border-[#FF6A2A] focus:ring-2 focus:ring-[#FF6A2A]/10 outline-none"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    className="w-full px-4 py-2.5 rounded-lg border border-white/10 text-sm text-[#F7F9FB] focus:border-[#FF6A2A] focus:ring-2 focus:ring-[#FF6A2A]/10 outline-none bg-[#162E3D]"
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <button type="button" onClick={addPackage} className="text-xs font-semibold text-[#FF6A2A] hover:text-[#FF8F5A]">+ Add Package</button>
                </div>
                {packages.length === 0 && (
                ...
                {packages.map(pkg => {
                  ...
                  return (
                    <div key={pkg.id} className="flex flex-wrap items-end gap-2 mb-2 p-3 bg-[#162E3D] rounded-lg border border-slate-100">
                      <div className="w-36">
                        <label className="block text-[10px] text-[#6B7C86] mb-0.5">Type</label>
                        <select
                          className="w-full px-2 py-1.5 rounded border border-white/10 text-xs bg-[#162E3D]"
                          value={pkg.type}
                      ...
                      <div className="w-16">
                        ...
                        <input type="number" min="1" className="w-full px-2 py-1.5 rounded border border-white/10 text-xs" value={pkg.quantity} onChange={e => updatePackage(pkg.id, 'quantity', parseInt(e.target.value) || 1)} />
                      </div>
                      <div className="w-20">
                        <label className="block text-[10px] text-[#6B7C86] mb-0.5">Wt (kg)</label>
                        <input type="number" min="0" step="0.1" className="w-full px-2 py-1.5 rounded border border-white/10 text-xs" value={pkg.weightKg} onChange={e => updatePackage(pkg.id, 'weightKg', parseFloat(e.target.value) || 0)} />
                      </div>
                      ...
                      <div className="w-16">
                        ...
                        <input type="number" min="1" max="2400" disabled={typeInfo?.isPallet} className={"w-full px-2 py-1.5 rounded border text-xs" + (typeInfo?.isPallet ? ' bg-[#102535] text-[#6B7C86] border-white/10 cursor-not-allowed' : ' border-white/10')} value={pkg.lengthCm} onChange={e => updatePackage(pkg.id, 'lengthCm', parseInt(e.target.value) || 0)} />
                      </div>
                      <div className="w-16">
                        <label className="block text-[10px] text-[#6B7C86] mb-0.5">W cm</label>
                        <input type="number" min="1" max="2400" disabled={typeInfo?.isPallet} className={"w-full px-2 py-1.5 rounded border text-xs" + (typeInfo?.isPallet ? ' bg-[#102535] text-[#6B7C86] border-white/10 cursor-not-allowed' : ' border-white/10')} value={pkg.widthCm} onChange={e => updatePackage(pkg.id, 'widthCm', parseInt(e.target.value) || 0)} />
                      </div>
                      <div className="w-16">
                        <label className="block text-[10px] text-[#6B7C86] mb-0.5">H cm{typeInfo?.isPallet ? ' ✎' : ''}</label>
                        <input type="number" min="1" max="2400" className="w-full px-2 py-1.5 rounded border border-white/10 text-xs" value={pkg.heightCm} onChange={e => updatePackage(pkg.id, 'heightCm', parseInt(e.target.value) || 0)} />
                      </div>
                      <div className="flex-1 min-w-[100px]">
                        <label className="block text-[10px] text-[#6B7C86] mb-0.5">Note</label>
                        <input type="text" className="w-full px-2 py-1.5 rounded border border-white/10 text-xs" placeholder="Optional" value={pkg.description} onChange={e => updatePackage(pkg.id, 'description', e.target.value)} />
                      </div>
              ...
              <div className="flex justify-end gap-3 pt-2">
                <button
                  ...
                  className="px-5 py-2.5 text-sm font-medium text-[#9AADB8] hover:bg-[#102535] rounded-lg transition-colors"
                >
                  Cancel
                </button>
                <button
        ...
        {/* Tabs */}
        <div className="flex gap-1 mb-6 bg-[#162E3D] rounded-xl shadow-sm border border-slate-100 p-1">
          <button
        ...
        {loading ? (
          ...
          <div className="bg-[#162E3D] rounded-xl shadow-sm border border-slate-100 p-16 text-center">
          ...
          <div className="space-y-4">
            {displayedQuotes.map(quote => {
              ...
              return (
                <div key={quote.id} className="bg-[#162E3D] rounded-xl shadow-sm border border-slate-100 overflow-hidden">
                  <div className="p-6">
                    ...
                    {quote.quotedPrice != null && (
                      <div className="bg-[#162E3D] rounded-lg p-4 border border-white/10 mb-4">
                        ...
                        {quote.responseMessage && (
                          <p className="text-sm text-[#F7F9FB] mt-3 pt-3 border-t border-white/10">
                            {quote.responseMessage}
                          </p>
                        )}
                      </div>
                    ...
                    <div className="flex items-center justify-between pt-4 border-t border-slate-100">
                      <div className="flex items-center gap-2 text-sm text-[#6B7C86]">
                        {otherParty && (
                          <>
                            <div className="w-6 h-6 rounded-full bg-[#162E3D] flex items-center justify-center flex-shrink-0">
                        ...
                        {(isRequester || isProvider) &&
                          (quote.status === 'PENDING' || quote.status === 'QUOTED') && (
                            <button
                              ...
                              disabled={actionLoading === quote.id}
                              className="px-3 py-1.5 text-xs font-medium text-red-400 hover:bg-red-900/20 rounded-lg transition-colors disabled:opacity-50"
                            >
                  ...
                  {isRespondingToThis && (
                    <div className="bg-[#162E3D] border-t border-slate-100 p-6">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
                        <div>
                          ...
                          <input
                            ...
                            min="0"
                            required
                            className="w-full px-3 py-2 rounded-lg border border-white/10 text-sm text-[#F7F9FB] focus:border-[#FF6A2A] focus:ring-2 focus:ring-[#FF6A2A]/10 outline-none bg-[#162E3D]"
                      ...
                      <div className="flex justify-end gap-2">
                        <button
                          ...
                          className="px-4 py-2 text-xs font-medium text-[#9AADB8] hover:bg-[#162E3D] rounded-lg transition-colors"
                        >
                          Cancel
                        </button>
                        <button
