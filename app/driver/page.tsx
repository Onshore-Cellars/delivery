...
export default function DriverPage() {
  ...
  if (!user.canCarry) {
    return (
      <div className="page-container narrow text-center py-20">
        <h2 className="text-xl font-bold text-[var(--c-ink)] mb-2">Driver Mode</h2>
        <p className="text-[var(--c-text-3)] mb-4">Enable &ldquo;I can carry / deliver&rdquo; in your profile to access driver mode.</p>
  ...
  return (
    <div className="page-container narrow">
      <div className="mb-6">
        <p className="text-[11px] font-semibold text-[var(--c-accent)] uppercase tracking-[0.15em] mb-1">Driver Mode</p>
        <h1 className="text-xl sm:text-2xl font-semibold text-[var(--c-ink)] tracking-[-0.02em]">Active Route</h1>
        <p className="text-sm text-[var(--c-text-3)] mt-1">Manage your deliveries and share live tracking with customers.</p>
      </div>
      ...
      {/* Tracking Status */}
      {tracking && position && (
        <div className="bg-[var(--c-success)]/10 border border-green-500/30 rounded-2xl p-4 mb-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="w-3 h-3 rounded-full bg-[var(--c-success)]/100 animate-pulse" />
              <div>
                <span className="text-sm font-semibold text-[var(--c-success)]">Live Tracking Active</span>
            ...
            <button
              ...
              className="text-xs font-semibold text-[var(--c-error)] hover:text-red-800 px-3 py-1.5 rounded-lg hover:bg-[#B23A2E]/10 transition-colors"
            >
              Stop Tracking
            </button>
          </div>
      ...
      {loading ? (
        ...
        <div className="bg-[var(--c-surface)] rounded-2xl border border-black/10 p-10 text-center">
          <div className="w-14 h-14 rounded-2xl bg-[var(--c-canvas-2)] flex items-center justify-center mx-auto mb-4">
            <svg className="w-6 h-6 text-[var(--c-text-2)]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 20l-5.447-2.724A1 1 0 013 16.382V5.618a1 1 0 011.447-.894L9 7m0 13l6-3m-6 3V7m6 10l4.553 2.276A1 1 0 0021 18.382V7.618a1 1 0 00-.553-.894L15 4m0 13V4m0 0L9 7" /></svg>
          </div>
          <p className="text-[var(--c-ink)] font-semibold mb-1.5">No active deliveries</p>
          <p className="text-sm text-[var(--c-text-3)] mb-5">Create a listing and get bookings to start delivering.</p>
        ...
        <div className="space-y-6">
          ...
          {!hasActiveTracking && uniqueListings.map(listingId => {
            ...
            return (
              <div key={listingId} className="bg-[var(--c-surface)] rounded-2xl border border-black/10 p-5">
                <h3 className="font-bold text-[var(--c-ink)] mb-1">{listingStops[0]?.listingTitle}</h3>
          ...
          <div className="space-y-3">
            <h2 className="text-sm font-semibold text-[var(--c-text-2)] uppercase tracking-wider">Delivery Stops</h2>
            {stops.map((stop, index) => (
              <div key={stop.id} className="bg-[var(--c-surface)] rounded-2xl border border-black/10 p-4 sm:p-5">
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <h3 className="text-sm font-bold text-[var(--c-ink)]">{stop.cargoDescription}</h3>
                      <p className="text-xs text-[var(--c-text-2)] mt-0.5">
                        {stop.weightKg}kg &middot; {stop.volumeM3}m&sup3;
                        {stop.routeDirection === 'return' && <span className="ml-1 text-[var(--c-accent)] font-medium">(Return)</span>}
                      </p>
                      {stop.yachtName && (
                        <p className="text-xs text-[var(--c-text-3)] mt-1">
                          ...
                          {stop.yachtMMSI && (
                            <a
                              ...
                              className="ml-1.5 text-[var(--c-accent)] hover:underline"
                            >
                              Track vessel
                            </a>
                          )}
                ...
                <div className="mt-4 pt-3 border-t border-black/10 flex flex-wrap gap-2">
                  {stop.status === 'CONFIRMED' && (
                    <button
                      onClick={() => advanceStatus(stop.id, 'PICKED_UP')}
                      disabled={advancing === stop.id + ':PICKED_UP'}
                      className="rounded-lg bg-[var(--c-info)] px-3.5 py-2 text-xs font-bold uppercase tracking-wide text-white hover:brightness-110 disabled:opacity-60"
                    >
                  ...
                  {(stop.status === 'PICKED_UP' || stop.status === 'IN_TRANSIT') && (
                    <button
                      ...
                      className="rounded-lg bg-[var(--c-success)] px-3.5 py-2 text-xs font-bold uppercase tracking-wide text-[var(--c-canvas)] hover:brightness-95"
                    >
                      Complete delivery
                    </button>
                  )}
                  {stop.status === 'DELIVERED' && (
                    <span className="inline-flex items-center gap-1.5 rounded-lg bg-[var(--c-success)]/15 px-3 py-2 text-xs font-bold uppercase tracking-wide text-[var(--c-success)]">
                      ✓ Delivered
                    </span>
                  )}
                </div>
          ...
          {hasActiveTracking && !tracking && (
            <button
              ...
              className="w-full py-4 rounded-2xl bg-[var(--c-success)] text-white font-semibold text-sm hover:bg-green-700 transition-colors flex items-center justify-center gap-2"
            >
