      ...
      <div className="w-full max-w-md">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input type="email" required className="w-full px-4 py-3 rounded-xl border border-white/[0.08] text-sm focus:border-[#1E6F8F] focus:ring-2 focus:ring-[#1E6F8F]/10 outline-none" value={email} onChange={e => setEmail(e.target.value)} placeholder="your@email.com" />
          </div>
          <button type="submit" disabled={loading} className="w-full btn-primary !py-3 disabled:opacity-50">
            {loading ? 'Sending...' : 'Send Reset Link'}
          </button>
          <div className="text-center">
            <Link href="/login" className="text-sm text-[#6B7C86] hover:text-[#F7F9FB]">Back to Sign In</Link>
          </div>
        </form>
      </div>
    </div>
