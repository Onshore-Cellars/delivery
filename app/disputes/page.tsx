  ...
  return (
    <div id="main-content" className="min-h-screen bg-[#faf9f7] py-8 px-4">
      <div className="max-w-4xl mx-auto">
        ...
        {loading ? (
          <div className="space-y-4">{[1,2,3].map(i => <div key={i} className="h-32 bg-white rounded-lg border border-[#e8e4de] animate-pulse" />)}</div>
        ) : disputes.length === 0 ? (
          <div className="text-center py-16 bg-white rounded-lg border border-[#e8e4de]">
            <p className="text-slate-500 mb-2">No disputes</p>
            <p className="text-xs text-slate-400">Disputes you raise or receive will appear here</p>
          </div>
        ) : (
          <div className="space-y-4">
            {disputes.map(d => (
              <div key={d.id} className="bg-white rounded-lg border border-[#e8e4de] p-5">
                <div className="flex items-start justify-between mb-3">
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      <span className="font-semibold text-[#1a1a1a]">{typeLabels[d.type] || d.type}</span>
                      <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${statusColors[d.status] || 'bg-slate-100 text-slate-500'}`}>
                        {d.status.replace('_', ' ')}
                      </span>
