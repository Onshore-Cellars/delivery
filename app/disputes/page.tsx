  ...
  return (
    <div id="main-content" className="min-h-screen bg-[#102535] py-8 px-4">
      <div className="max-w-4xl mx-auto">
        ...
        {loading ? (
          <div className="space-y-4">{[1,2,3].map(i => <div key={i} className="h-32 bg-[#162E3D] rounded-lg border border-white/[0.08] animate-pulse" />)}</div>
        ) : disputes.length === 0 ? (
          <div className="text-center py-16 bg-[#162E3D] rounded-lg border border-white/[0.08]">
            <p className="text-[#6B7C86] mb-2">No disputes</p>
            <p className="text-xs text-[#6B7C86]">Disputes you raise or receive will appear here</p>
          </div>
        ) : (
          <div className="space-y-4">
            {disputes.map(d => (
              <div key={d.id} className="bg-[#162E3D] rounded-lg border border-white/[0.08] p-5">
                <div className="flex items-start justify-between mb-3">
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      {d.priority && <span className="text-xs text-[#6B7C86]">Priority: {d.priority}</span>}
                    </div>
                    {d.booking.trackingCode && (
                      <p className="text-xs text-[#6B7C86]">Booking: {d.booking.trackingCode}</p>
                    )}
                  </div>
                  <span className="text-xs text-[#6B7C86]">{new Date(d.createdAt).toLocaleDateString()}</span>
                </div>
                ...
                {user?.role === 'ADMIN' && (
                  <div className="mt-3 pt-3 border-t border-white/[0.08]">
                    {assessments[d.id] ? (
                      <div className="bg-[#102535] rounded p-3 text-xs space-y-1">
                      ...
                      <button
                        ...
                        className="px-3 py-1.5 bg-[#FF6A2A] text-white rounded text-xs font-medium hover:bg-[#E85A1E] disabled:opacity-50 transition-colors"
                      >
