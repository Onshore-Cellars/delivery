...
export default function AboutPage() {
  return (
    <div className="page-container narrow">
        <p className="text-[11px] font-semibold text-[var(--c-accent)] uppercase tracking-[0.15em] mb-1.5">About Us</p>
        <h1 className="text-2xl sm:text-3xl font-semibold text-[var(--c-ink)] tracking-[-0.02em] mb-8 sm:mb-10">About Onshore Deliver</h1>

        {/* The Problem */}
        <div className="bg-[var(--c-surface)] rounded-2xl shadow-[0_2px_8px_rgba(0,0,0,0.3)] border border-black/10 p-6 sm:p-10 mb-8 sm:mb-10">
          <h2 className="text-lg sm:text-xl font-bold text-[var(--c-ink)] mb-4">The Problem We Solve</h2>
          <p className="text-sm sm:text-base text-[var(--c-text-2)] leading-relaxed mb-4">
          ...
          <p className="text-sm sm:text-base text-[var(--c-text-2)] leading-relaxed">
        ...
        {/* Two-Way Routes */}
        <div className="bg-[var(--c-canvas)] rounded-2xl border border-black/10 p-6 sm:p-10 mb-8 sm:mb-10">
          <div className="flex items-center gap-3 mb-4">
            <div className="w-10 h-10 rounded-full bg-[var(--c-accent)]/10 flex items-center justify-center flex-shrink-0">
              <svg className="w-5 h-5 text-[var(--c-accent)]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4" />
              </svg>
            </div>
            <h2 className="text-lg sm:text-xl font-bold text-[var(--c-ink)]">Two-Way Routes &mdash; No Empty Returns</h2>
          </div>
          <p className="text-sm sm:text-base text-[var(--c-text-2)] leading-relaxed mb-4">
        ...
        {/* Who We Serve */}
        <div className="bg-[var(--c-surface)] rounded-2xl shadow-[0_2px_8px_rgba(0,0,0,0.3)] border border-black/10 p-6 sm:p-10 mb-8 sm:mb-10">
          <h2 className="text-lg sm:text-xl font-bold text-[var(--c-ink)] mb-4">Built for the People Who Run Yachts</h2>
          <p className="text-sm sm:text-base text-[var(--c-text-2)] leading-relaxed mb-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {[
              ...
              <div key={item.role} className="p-4 rounded-xl bg-[var(--c-canvas)] border border-black/10">
                <h3 className="font-semibold text-[var(--c-ink)] text-sm mb-1">{item.role}</h3>
                <p className="text-xs text-[var(--c-text-3)] leading-relaxed">{item.desc}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6 mb-8 sm:mb-10">
          {[
            ...
            <div key={item.label} className="bg-[var(--c-surface)] rounded-2xl shadow-[0_2px_8px_rgba(0,0,0,0.3)] border border-black/10 p-6 sm:p-8 text-center hover:shadow-[0_4px_12px_rgba(0,0,0,0.06)] transition-shadow">
              <div className="text-3xl sm:text-4xl font-semibold text-[var(--c-ink)] tracking-[-0.02em]">{item.stat}</div>
              <div className="text-sm font-semibold text-[var(--c-accent)] mt-1">{item.label}</div>
              <p className="text-xs text-[var(--c-text-3)] mt-2">{item.desc}</p>
            </div>
          ))}
        </div>

        <div className="bg-[var(--c-surface)] rounded-2xl shadow-[0_2px_8px_rgba(0,0,0,0.3)] border border-black/10 p-6 sm:p-10 mb-8 sm:mb-10">
          <h2 className="text-lg sm:text-xl font-bold text-[var(--c-ink)] mb-6">How It Works</h2>
          <div className="space-y-6">
            <div className="flex gap-4">
              <div className="w-10 h-10 rounded-full bg-[var(--c-surface)] flex items-center justify-center flex-shrink-0">
                <span className="text-sm font-bold text-[var(--c-ink)]">1</span>
              </div>
              <div>
                <h3 className="font-semibold text-[var(--c-ink)]">Carriers List Spare Space &mdash; Both Ways</h3>
                <p className="text-sm text-[var(--c-text-3)] mt-1">Driving to a port or marina? List your spare capacity on the outbound run and on the return. Set pricing for each direction.</p>
              </div>
            </div>
            <div className="flex gap-4">
              <div className="w-10 h-10 rounded-full bg-[var(--c-surface)] flex items-center justify-center flex-shrink-0">
                <span className="text-sm font-bold text-[var(--c-ink)]">2</span>
              </div>
              <div>
                <h3 className="font-semibold text-[var(--c-ink)]">Crew, Vendors &amp; Managers Book Space</h3>
                <p className="text-sm text-[var(--c-text-3)] mt-1">Browse available routes, book capacity for supplies going to the yacht or items coming back. Request a quote for custom loads.</p>
              </div>
            </div>
            <div className="flex gap-4">
              <div className="w-10 h-10 rounded-full bg-[var(--c-surface)] flex items-center justify-center flex-shrink-0">
                <span className="text-sm font-bold text-[var(--c-ink)]">3</span>
              </div>
              <div>
                <h3 className="font-semibold text-[var(--c-ink)]">Track Live, Delivered Dockside</h3>
                <p className="text-sm text-[var(--c-text-3)] mt-1">Drivers start their route from the app. You see live position, ETA, and get SMS updates. Delivery confirmed with signature and photo at the berth.</p>
              </div>
            </div>
          </div>
        </div>
        ...
        {/* Platform Features */}
        <div className="bg-[var(--c-surface)] rounded-2xl shadow-[0_2px_8px_rgba(0,0,0,0.3)] border border-black/10 p-6 sm:p-10 mb-8 sm:mb-10">
          <h2 className="text-lg sm:text-xl font-bold text-[var(--c-ink)] mb-6">Platform Features</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {[
              ...
              <div key={item.feat} className="flex items-start gap-3 p-3 rounded-lg">
                ...
                <div>
                  <h3 className="text-sm font-semibold text-[var(--c-ink)]">{item.feat}</h3>
                  <p className="text-xs text-[var(--c-text-3)] mt-0.5">{item.desc}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="bg-[var(--c-accent)] rounded-2xl p-6 sm:p-10 text-center relative overflow-hidden">
          ...
          <p className="text-sm sm:text-base text-[var(--c-text-3)] mb-6">Join crew, vendors, fleet managers, and carriers who are making maritime logistics smarter.</p>
          <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center">
            <Link href="/register" className="btn-primary text-sm !py-3 !px-8">Create Free Account</Link>
            <Link href="/marketplace" className="inline-flex items-center justify-center px-8 py-3 rounded-xl text-sm font-semibold text-white border border-black/20 hover:bg-[var(--c-surface)]/5 transition-all">Browse Marketplace</Link>
          </div>
          </div>
        </div>
      </div>
