    ...
    <>
      ...
      {open && (
        <div className="fixed bottom-24 right-4 z-50 w-[calc(100vw-2rem)] sm:w-96 h-[500px] bg-[var(--c-surface)] rounded-2xl shadow-[0_8px_30px_rgba(0,0,0,0.12)] border border-black/10 flex flex-col overflow-hidden">
          {/* Header */}
          <div className="flex items-center justify-between px-4 py-3 border-b border-[var(--c-border)] bg-[var(--c-accent)]">
            ...
            <button
              ...
              className="w-7 h-7 flex items-center justify-center rounded-full hover:bg-[var(--c-surface)]/10 transition-colors"
            >
          ...
          {/* Input */}
          <form onSubmit={sendMessage} className="p-3 border-t border-[var(--c-border)]">
            <div className="flex gap-2">
              <input
                ref={inputRef}
                type="text"
                className="flex-1 px-3.5 py-2 rounded-xl border border-black/10 text-sm text-[var(--c-ink)] placeholder-slate-400 focus:border-[var(--c-accent)] focus:ring-2 focus:ring-[var(--c-accent)]/10 outline-none"
              ...
              <button
                ...
                className="flex items-center justify-center w-9 h-9 rounded-xl bg-[var(--c-accent)] text-white hover:bg-[var(--c-accent-hover)] disabled:opacity-50 transition-colors flex-shrink-0"
              >
      ...
      <button
        ...
        className="fixed bottom-6 right-4 z-50 w-14 h-14 rounded-full bg-[var(--c-accent)] text-white shadow-lg hover:scale-105 transition-transform flex items-center justify-center"
      >
        {open ? (
        ...
        {unread && !open && (
          <span className="absolute top-0 right-0 w-3 h-3 bg-[var(--c-accent)] rounded-full border-2 border-[var(--c-canvas)]" />
        )}
      </button>
    </>
  )
