...
export default function AIChatWidget() {
  ...
  return (
    <>
      {/* Chat Panel */}
      {open && (
        <div className="fixed bottom-24 right-4 z-50 w-[calc(100vw-2rem)] sm:w-96 h-[500px] bg-white rounded-2xl shadow-[0_8px_30px_rgba(0,0,0,0.12)] border border-[#e8e4de] flex flex-col overflow-hidden">
          {/* Header */}
            ...
            {messages.map((msg, i) => (
              <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[80%] px-3.5 py-2 rounded-2xl text-sm leading-relaxed ${
                  msg.role === 'user'
                    ? 'bg-[#C6904D] text-white rounded-br-md'
          ...
          <form onSubmit={sendMessage} className="p-3 border-t border-slate-100">
            <div className="flex gap-2">
              <input
                ref={inputRef}
                type="text"
                className="flex-1 px-3.5 py-2 rounded-xl border border-slate-200 text-sm text-[#1a1a1a] placeholder-slate-400 focus:border-[#C6904D] focus:ring-2 focus:ring-[#C6904D]/10 outline-none"
              ...
              <button
                ...
                className="flex items-center justify-center w-9 h-9 rounded-xl bg-[#C6904D] text-white hover:bg-[#b5813f] disabled:opacity-50 transition-colors flex-shrink-0"
              >
      ...
      >
        ...
        {unread && !open && (
          <span className="absolute top-0 right-0 w-3 h-3 bg-[#C6904D] rounded-full border-2 border-[#1a1a1a]" />
        )}
      </button>
    </>
  )
