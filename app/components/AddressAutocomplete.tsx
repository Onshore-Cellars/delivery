...
export default function AddressAutocomplete({
  ...
  return (
    <div ref={containerRef} className={`relative ${className}`}>
      {label && <label className="block text-sm font-semibold text-[#1d1916] mb-2">{label}</label>}
      <div className="relative">
        <input
          ...
          autoComplete="off"
          className="w-full px-4 py-3 rounded border border-[#DED7CC] text-sm text-[#1d1916] focus:border-[#C6904D] focus:ring-2 focus:ring-[#C6904D]/10 outline-none pr-10"
        />
        <div className="absolute right-3 top-1/2 -translate-y-1/2">
          {loading ? (
            <svg className="w-4 h-4 text-stone-400 animate-spin" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" fill="none" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" /></svg>
          ) : (
      ...
      {open && (predictions.length > 0 || results.length > 0) && (
        <div className="absolute z-50 w-full mt-1 bg-white rounded-lg shadow-lg border border-[#DED7CC] overflow-hidden max-h-64 overflow-y-auto">
          {predictions.length > 0 ? predictions.map((pred, i) => (
            <button
              key={pred.placeId || i}
              type="button"
              onClick={() => selectGooglePlace(pred)}
              className="w-full text-left px-4 py-3 hover:bg-[#FAF8F4] transition-colors border-b border-[#EAE4DA] last:border-0"
            >
              <div className="flex items-start gap-2.5">
                <svg className="w-4 h-4 text-[#C6904D] mt-0.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
                <div>
                  <span className="text-sm text-[#1d1916] font-medium block">{pred.mainText}</span>
                  <span className="text-xs text-stone-500">{pred.secondaryText}</span>
                </div>
              </div>
            </button>
            ...
            >
              <div className="flex items-start gap-2.5">
                <svg className="w-4 h-4 text-[#C6904D] mt-0.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
                <div>
                  <span className="text-sm text-[#1d1916] font-medium block">{result.street || result.city}</span>
          ...
          <div className="px-4 py-2 text-[10px] text-stone-400 text-center bg-[#FAF8F4]">
