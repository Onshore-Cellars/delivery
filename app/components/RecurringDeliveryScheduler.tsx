        ...
        <div className="p-5 space-y-5">
              ...
              {(['weekly', 'biweekly', 'monthly'] as const).map(f => (
                <button
                  ...
                  className={`py-2.5 px-3 rounded-lg text-xs font-semibold border-2 transition-all ${
                    frequency === f ? 'border-[#C6904D] bg-[#FF6A2A]/5 text-[#F7F9FB]' : 'border-white/[0.08] text-[#4a4a4a]'
                  }`}
                >
          ...
          {frequency !== 'monthly' ? (
                ...
                {DAYS.map((day, i) => (
                  <button
                    ...
                    className={`flex-1 py-2 rounded-lg text-xs font-semibold transition-all ${
                      dayOfWeek === i ? 'bg-[#FF6A2A] text-white' : 'bg-[#102535] text-[#4a4a4a] hover:bg-[#e8e4de]'
                    }`}
                  >
                    {day}
                  </button>
            ...
            <div>
              ...
              <select
                ...
                className="w-full px-3 py-2.5 rounded border border-white/[0.08] text-sm focus:border-[#1E6F8F] outline-none"
              >
              ...
              {[
                ...
                <button
                  ...
                  className={`py-2 px-3 rounded-lg text-xs font-semibold border-2 transition-all ${
                    timeWindow === w.v ? 'border-[#C6904D] bg-[#FF6A2A]/5' : 'border-white/[0.08]'
                  }`}
                >
                  {w.l} <span className="text-[#6B7C86] font-normal">({w.t})</span>
                </button>
              ))}
            </div>
          </div>
            ...
            <div className="space-y-2">
              <label className="flex items-center gap-2 text-sm">
                ...
                <input type="number" min={2} max={52} value={count || 4} onChange={e => setCount(Number(e.target.value))}
                  className="w-16 px-2 py-1 rounded border border-white/[0.08] text-sm text-center focus:border-[#1E6F8F] outline-none" />
              ...
              <label className="flex items-center gap-2 text-sm">
                ...
                <input type="date" value={endDate} onChange={e => setEndDate(e.target.value)}
                  className="flex-1 px-2 py-1 rounded border border-white/[0.08] text-sm focus:border-[#1E6F8F] outline-none" />
              </label>
              <label className="flex items-center gap-2 text-sm">
          ...
          <div>
            ...
            <textarea value={notes} onChange={e => setNotes(e.target.value)} placeholder="e.g. Weekly provisions run"
              className="w-full px-3 py-2.5 rounded border border-white/[0.08] text-sm focus:border-[#1E6F8F] outline-none min-h-[50px] resize-none" />
          </div>

          {/* Preview */}
          {startDate && (
            <div className="bg-[#102535] rounded-lg p-4">
