        ...
        <div className="p-5 space-y-5">
              ...
              {(['weekly', 'biweekly', 'monthly'] as const).map(f => (
                <button
                  ...
                  className={`py-2.5 px-3 rounded-lg text-xs font-semibold border-2 transition-all ${
                    frequency === f ? 'border-[#FF6A2A] bg-[#FF6A2A]/5 text-[#1a1a1a]' : 'border-[#e8e4de] text-[#4a4a4a]'
                  }`}
                >
          ...
          {frequency !== 'monthly' ? (
                ...
                {DAYS.map((day, i) => (
                  <button
                    ...
                    className={`flex-1 py-2 rounded-lg text-xs font-semibold transition-all ${
                      dayOfWeek === i ? 'bg-[#FF6A2A] text-white' : 'bg-[#f5f3f0] text-[#4a4a4a] hover:bg-[#e8e4de]'
                    }`}
                  >
                    {day}
                  </button>
            ...
            <div>
              ...
              <select
                ...
                className="w-full px-3 py-2.5 rounded border border-[#e8e4de] text-sm focus:border-[#FF6A2A] outline-none"
              >
              ...
              {[
                ...
                <button
                  ...
                  className={`py-2 px-3 rounded-lg text-xs font-semibold border-2 transition-all ${
                    timeWindow === w.v ? 'border-[#FF6A2A] bg-[#FF6A2A]/5' : 'border-[#e8e4de]'
                  }`}
                >
            ...
            <div className="space-y-2">
              <label className="flex items-center gap-2 text-sm">
                ...
                <input type="number" min={2} max={52} value={count || 4} onChange={e => setCount(Number(e.target.value))}
                  className="w-16 px-2 py-1 rounded border border-[#e8e4de] text-sm text-center focus:border-[#FF6A2A] outline-none" />
              ...
              <label className="flex items-center gap-2 text-sm">
                ...
                <input type="date" value={endDate} onChange={e => setEndDate(e.target.value)}
                  className="flex-1 px-2 py-1 rounded border border-[#e8e4de] text-sm focus:border-[#FF6A2A] outline-none" />
              </label>
              <label className="flex items-center gap-2 text-sm">
          ...
          <div>
            ...
            <textarea value={notes} onChange={e => setNotes(e.target.value)} placeholder="e.g. Weekly provisions run"
              className="w-full px-3 py-2.5 rounded border border-[#e8e4de] text-sm focus:border-[#FF6A2A] outline-none min-h-[50px] resize-none" />
          </div>
