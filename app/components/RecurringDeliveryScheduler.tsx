...
export default function RecurringDeliveryScheduler({ onSchedule, onClose }: RecurringDeliverySchedulerProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
      ...
      <div className="relative bg-[var(--c-surface)] w-full sm:max-w-md sm:rounded-lg rounded-t-2xl shadow-2xl max-h-[90vh] overflow-y-auto">
        <div className="sticky top-0 bg-[var(--c-surface)] border-b border-black/10 px-5 py-4 z-10 rounded-t-2xl sm:rounded-t-lg flex items-center justify-between">
          <h2 className="text-lg font-semibold text-[var(--c-ink)]" style={{ fontFamily: 'var(--font-display)' }}>Recurring Schedule</h2>
        ...
        <div className="p-5 space-y-5">
          {/* Frequency */}
          <div>
            <label className="block text-sm font-semibold text-[var(--c-ink)] mb-2">Frequency</label>
              ...
              {(['weekly', 'biweekly', 'monthly'] as const).map(f => (
                <button
                  ...
                  className={`py-2.5 px-3 rounded-lg text-xs font-semibold border-2 transition-all ${
                    frequency === f ? 'border-[var(--c-accent)] bg-[var(--c-accent)]/5 text-[var(--c-ink)]' : 'border-black/10 text-[var(--c-text-2)]'
                  }`}
                >
          ...
          {frequency !== 'monthly' ? (
            <div>
              <label className="block text-sm font-semibold text-[var(--c-ink)] mb-2">Day of Week</label>
            ...
            <div>
              <label className="block text-sm font-semibold text-[var(--c-ink)] mb-2">Day of Month</label>
              <select
                value={dayOfMonth}
                onChange={e => setDayOfMonth(Number(e.target.value))}
                className="w-full px-3 py-2.5 rounded border border-black/10 text-sm focus:border-[var(--c-accent)] outline-none"
              >
          ...
          {/* Time window */}
          <div>
            <label className="block text-sm font-semibold text-[var(--c-ink)] mb-2">Time Window</label>
              ...
              {[
                ...
                <button
                  ...
                  className={`py-2 px-3 rounded-lg text-xs font-semibold border-2 transition-all ${
                    timeWindow === w.v ? 'border-[var(--c-accent)] bg-[var(--c-accent)]/5' : 'border-black/10'
                  }`}
                >
                  {w.l} <span className="text-[var(--c-text-2)] font-normal">({w.t})</span>
                </button>
              ))}
            </div>
          </div>
          ...
          {/* Start date */}
          <div>
            <label className="block text-sm font-semibold text-[var(--c-ink)] mb-2">Start Date</label>
          ...
          {/* End condition */}
          <div>
            <label className="block text-sm font-semibold text-[var(--c-ink)] mb-2">Ends</label>
            <div className="space-y-2">
              <label className="flex items-center gap-2 text-sm">
                ...
                <input type="number" min={2} max={52} value={count || 4} onChange={e => setCount(Number(e.target.value))}
                  className="w-16 px-2 py-1 rounded border border-black/10 text-sm text-center focus:border-[var(--c-accent)] outline-none" />
              ...
              <label className="flex items-center gap-2 text-sm">
                ...
                <input type="date" value={endDate} onChange={e => setEndDate(e.target.value)}
                  className="flex-1 px-2 py-1 rounded border border-black/10 text-sm focus:border-[var(--c-accent)] outline-none" />
              </label>
              <label className="flex items-center gap-2 text-sm">
          ...
          <div>
            ...
            <textarea value={notes} onChange={e => setNotes(e.target.value)} placeholder="e.g. Weekly provisions run"
              className="w-full px-3 py-2.5 rounded border border-black/10 text-sm focus:border-[var(--c-accent)] outline-none min-h-[50px] resize-none" />
          </div>

          {/* Preview */}
          {startDate && (
            <div className="bg-[var(--c-canvas)] rounded-lg p-4">
              <p className="text-xs font-semibold text-[var(--c-accent)] uppercase tracking-wider mb-2">Preview — next deliveries</p>
              <div className="space-y-1.5">
                {getPreviewDates().map((d, i) => (
                  <div key={i} className="flex items-center gap-2 text-xs">
                    <span className="w-5 h-5 rounded-full bg-[var(--c-accent)]/10 text-[var(--c-accent)] flex items-center justify-center font-semibold text-[10px]">{i + 1}</span>
                    <span className="text-[var(--c-ink)] font-medium">
