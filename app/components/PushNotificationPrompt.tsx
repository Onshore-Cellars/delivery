...
export default function PushNotificationPrompt({ token }: PushNotificationPromptProps) {
  ...
  return (
    <div className="fixed bottom-24 md:bottom-6 left-4 right-4 md:left-auto md:right-6 z-50 md:w-80 animate-slideUp">
      <div className="bg-[var(--c-surface)] rounded-xl shadow-2xl border border-black/10 p-4">
        <div className="flex items-start gap-3">
          <div className="w-10 h-10 rounded-full bg-[var(--c-accent)]/10 flex items-center justify-center flex-shrink-0">
            <svg className="w-5 h-5 text-[var(--c-accent)]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" /></svg>
          </div>
          <div className="flex-1">
            <h4 className="text-sm font-semibold text-[var(--c-ink)]">Stay Updated</h4>
            <p className="text-xs text-[var(--c-text-3)] mt-0.5">Get notified about delivery updates, new messages, and bids.</p>
          </div>
          <button onClick={handleDismiss} className="text-[var(--c-text-2)] hover:text-[var(--c-text-2)] p-1">
