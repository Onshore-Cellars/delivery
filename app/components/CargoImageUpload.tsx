...
export default function CargoImageUpload({ images, onChange, maxImages = 5 }: CargoImageUploadProps) {
  ...
  return (
    <div>
      <div className="flex flex-wrap gap-3">
        {images.map((img, i) => (
          <div key={i} className="relative w-20 h-20 rounded-xl overflow-hidden border border-black/10 group">
            ...
            <button
              ...
              className="absolute top-0.5 right-0.5 w-5 h-5 rounded-full bg-[var(--c-error)] text-white flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity"
            >
        ...
        {images.length < maxImages && (
          <button
            ...
            disabled={uploading}
            className="w-20 h-20 rounded-xl border-2 border-dashed border-black/15 flex flex-col items-center justify-center text-[var(--c-text-2)] hover:border-[var(--c-accent)] hover:text-[var(--c-accent)] transition-colors disabled:opacity-50"
          >
            {uploading ? (
      ...
      <p className="text-[10px] text-[var(--c-text-2)] mt-1.5">
        {images.length}/{maxImages} photos &middot; Optional &middot; Max 8MB each
      </p>
      {error && <p className="text-xs text-[var(--c-error)] mt-1">{error}</p>}
