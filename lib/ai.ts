...
// ─── BILL EXTRACTION (PDF/image → structured bill) ─────────────────────────
...
export interface ExtractedBill {
  ...
  total: number | null
  category: string | null      // fuel, provisions, berthing, maintenance, transport, other
  notes: string | null
...
/**
 * Read a supplier bill / invoice (PDF or image) and extract structured fields.
 * Results are a DRAFT for the user to review & correct before saving.
 */
export async function extractBillDocument(base64Data: string, mediaType: string): Promise<ExtractedBill | null> {
  if (!process.env.ANTHROPIC_API_KEY) return null
  const system = `You extract data from supplier bills/invoices for a maritime logistics platform. Read the attached document and return ONLY valid JSON (no markdown) with this exact shape:
{"supplierName":string|null,"invoiceNumber":string|null,"invoiceDate":"YYYY-MM-DD"|null,"dueDate":"YYYY-MM-DD"|null,"currency":"EUR"|"GBP"|...|null,"subtotal":number|null,"taxAmount":number|null,"total":number|null,"category":"fuel"|"provisions"|"berthing"|"maintenance"|"transport"|"other"|null,"notes":string|null,"lineItems":[{"description":string,"quantity":number|null,"unitPrice":number|null,"amount":number|null}],"confidence":"high"|"medium"|"low"}
Rules: amounts as plain numbers (no thousands separators). taxAmount = total VAT/tax. Use null for anything not clearly present — never guess. "confidence" reflects how legible/complete the document was. "notes" = anything unusual worth flagging (handwriting, missing totals, multiple currencies).`
  try {
    ...
    const msg = await client.messages.create({
      ...
      max_tokens: 3000,
      system,
      messages: [{ role: 'user', content: [source, { type: 'text', text: 'Extract the bill data as JSON.' }] }],
    })
    const block = msg.content[0]
